(function(){
  const apiRoot = (window.API_BASE && window.API_BASE.indexOf('http')===0) ? window.API_BASE + '/api/mock' : window.location.origin + '/api/mock';
  ...
  async function fetchLedgers(){
    const r = await fetch(apiRoot + '/ledgers');
  ...
  async function openLedger(id){
    currentLedger = id;
    // fetch ledger meta and transactions from raw endpoint
    const r = await fetch(apiRoot.replace('/api/mock','') + '/api/mock/raw');
